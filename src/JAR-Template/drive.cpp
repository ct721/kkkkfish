...
{
    this->is_smooth_stopping = false; // Initialize the new flag
...
void Drive::control_arcade(){
  float throttle = deadband(controller(primary).Axis3.value(), 0.05) * driver_scale;
  float turn = deadband(controller(primary).Axis1.value(), 0.05) * driver_scale;

  float current_left_rpm = DriveL.velocity(vex::velocityUnits::rpm);
  float current_right_rpm = DriveR.velocity(vex::velocityUnits::rpm);
  float average_rpm = (current_left_rpm + current_right_rpm) / 2.0;

  const float DECELERATION_THRESHOLD_RPM = 50.0; // Adjust as needed
  const float REVERSE_INPUT_THRESHOLD = -20.0; // Adjust as needed (joystick value)

  if (!is_smooth_stopping) {
    // Detect high-speed forward movement and sudden reverse input
    if (average_rpm > DECELERATION_THRESHOLD_RPM && throttle < REVERSE_INPUT_THRESHOLD) {
      is_smooth_stopping = true; // Indicate that smooth stop is starting
      smooth_stop_task(*this); // This call blocks until the robot stops
      is_smooth_stopping = false; // Reset after smooth_stop_task completes
    } else {
      // Normal arcade control
      DriveL.spin(vex::directionType::fwd, to_volt(throttle+turn), vex::voltageUnits::volt);
      DriveR.spin(vex::directionType::fwd, to_volt(throttle-turn), vex::voltageUnits::volt);
    }
  } else {
    // If a smooth stop is in progress (shouldn't happen if smooth_stop_task is blocking
    // but as a safeguard, if we enter this, it means smooth_stop_task is still running
    // or hasn't reset the flag properly. For a blocking call, this block would ideally not be reached.
    // If it is reached, it implies some non-blocking behavior or error in flag management.
    // To be safe, we'll let `smooth_stop_task` manage motor control when `is_smooth_stopping` is true
    // and ensure `control_arcade` doesn't interfere.
    // No motor commands from `control_arcade` in this state.
  }
}
...
void Drive::control_holonomic(){
  float throttle = deadband(controller(primary).Axis3.value(), 5) * driver_scale;
  float turn = deadband(controller(primary).Axis1.value(), 5) * driver_scale;
  float strafe = deadband(controller(primary).Axis4.value(), 5) * driver_scale;

  float current_lf_rpm = DriveLF.velocity(vex::velocityUnits::rpm);
  float current_rf_rpm = DriveRF.velocity(vex::velocityUnits::rpm);
  float current_lb_rpm = DriveLB.velocity(vex::velocityUnits::rpm);
  float current_rb_rpm = DriveRB.velocity(vex::velocityUnits::rpm);
  float average_rpm = (current_lf_rpm + current_rf_rpm + current_lb_rpm + current_rb_rpm) / 4.0;
  ...
  if (!is_smooth_stopping) {
    ...
    if (average_rpm > DECELERATION_THRESHOLD_RPM && throttle < REVERSE_INPUT_THRESHOLD) {
      ...
      // Normal holonomic control
      DriveLF.spin(vex::directionType::fwd, to_volt(throttle+turn+strafe), vex::voltageUnits::volt);
      DriveRF.spin(vex::directionType::fwd, to_volt(throttle-turn-strafe), vex::voltageUnits::volt);
      DriveLB.spin(vex::directionType::fwd, to_volt(throttle+turn-strafe), vex::voltageUnits::volt);
      DriveRB.spin(vex::directionType::fwd, to_volt(throttle-turn+strafe), vex::voltageUnits::volt);
    }
  } else {
    // If a smooth stop is in progress, do nothing here and let smooth_stop_task manage motors.
  }
}
...
void Drive::control_tank(){
  float leftthrottle = deadband(controller(primary).Axis3.value(), 0.05) * driver_scale;
  float rightthrottle = deadband(controller(primary).Axis2.value(), 0.05) * driver_scale;
  ...
  if (!is_smooth_stopping) {
    // Detect high-speed forward movement and sudden reverse input
    if (average_rpm > DECELERATION_THRESHOLD_RPM && leftthrottle < REVERSE_INPUT_THRESHOLD && rightthrottle < REVERSE_INPUT_THRESHOLD) {
      is_smooth_stopping = true; // Indicate that smooth stop is starting
      smooth_stop_task(*this); // This call blocks until the robot stops
      is_smooth_stopping = false; // Reset after smooth_stop_task completes
    } else {
      // Normal tank control
      DriveL.spin(vex::directionType::fwd, to_volt(leftthrottle), vex::voltageUnits::volt);
      DriveR.spin(vex::directionType::fwd, to_volt(rightthrottle), vex::voltageUnits::volt);
    }
  } else {
    // If a smooth stop is in progress, do nothing here and let smooth_stop_task manage motors.
  }
...
void Drive::set_driver_scale(float scale){
...
void smooth_stop_task(Drive &drive_obj) {
  const float DECELERATION_RAMP_RATE = 0.5; // Voltage reduction per cycle (adjust as needed)
  const float MIN_RPM_TO_STOP = 10.0;     // RPM threshold to consider robot stopped

  float current_left_voltage = drive_obj.DriveL.voltage(vex::voltageUnits::volt);
  float current_right_voltage = drive_obj.DriveR.voltage(vex::voltageUnits::volt);

  // Ensure we are decelerating in the correct direction if currently moving forward
  // If the robot is moving backward and the driver commanded forward, we still want to slow down if the current direction is opposite to the command
  // For smooth stop, we're assuming the robot is moving forward and we want to stop it.

  while (fabs(drive_obj.DriveL.velocity(vex::velocityUnits::rpm)) > MIN_RPM_TO_STOP || 
         fabs(drive_obj.DriveR.velocity(vex::velocityUnits::rpm)) > MIN_RPM_TO_STOP) {

    float average_rpm = (drive_obj.DriveL.velocity(vex::velocityUnits::rpm) + drive_obj.DriveR.velocity(vex::velocityUnits::rpm)) / 2.0;

    // Reduce voltage gradually, maintaining current direction
    if (average_rpm > 0) { // Moving forward
      current_left_voltage = fmax(0.0, current_left_voltage - DECELERATION_RAMP_RATE);
      current_right_voltage = fmax(0.0, current_right_voltage - DECELERATION_RAMP_RATE);
    } else { // Moving backward
      current_left_voltage = fmin(0.0, current_left_voltage + DECELERATION_RAMP_RATE);
      current_right_voltage = fmin(0.0, current_right_voltage + DECELERATION_RAMP_RATE);
    }

    drive_obj.drive_with_voltage(current_left_voltage, current_right_voltage);
    vex::task::sleep(10);
  }
  drive_obj.drive_stop(vex::brakeType::coast);
}